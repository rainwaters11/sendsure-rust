...
const APP_JS: &str = r#"const result=document.getElementById('result');const cont=document.getElementById('continue');const form=document.getElementById('intent-form');const evaluateButton=document.getElementById('evaluate');const resetButton=document.getElementById('reset');const scenarioBox=document.getElementById('scenarios');const checkButton=document.getElementById('check');const actionButtons=[...document.querySelectorAll('.actions button')];const evaluateDefaultLabel=(evaluateButton.textContent||'Evaluate intent').trim();const resultDefault='Choose a demo scenario or enter transaction details to begin.';const evaluatingText='Evaluating with deterministic Rust rules...';const fieldNames=['action_type','source_network','destination_network','asset_symbol','asset_identifier','destination_address','expected_destination_address','entered_destination_tag_or_memo','expected_destination_tag_or_memo','contract_address','approval_amount_or_scope','swap_slippage_percent','transaction_origin'];const allIntentFields=[...fieldNames,'asset_was_unsolicited'];const fieldVisibility={SEND:['source_network','destination_network','asset_symbol','asset_identifier','destination_address','expected_destination_address','entered_destination_tag_or_memo','expected_destination_tag_or_memo','transaction_origin','asset_was_unsolicited'],SWAP:['source_network','destination_network','asset_symbol','asset_identifier','swap_slippage_percent','transaction_origin'],APPROVE:['source_network','asset_symbol','asset_identifier','contract_address','approval_amount_or_scope','transaction_origin'],SIGN:['source_network','contract_address','transaction_origin','asset_was_unsolicited']};const formFields=Object.fromEntries([...form.elements].filter(el=>el.name).map(el=>[el.name,el]));let selectedScenarioIndex=null;let scenarioButtons=[];function blank(v){return v==null?'':String(v)}function nullableText(value){if(value==null)return null;const trimmed=String(value).trim();return trimmed===''?null:trimmed}function nullableNumber(value){const text=nullableText(value);if(text==null)return null;const parsed=Number(text);return Number.isFinite(parsed)?parsed:null}function fieldContainer(field){if(!field)return null;return field.type==='checkbox'&&field.parentElement?field.parentElement:field}function clearFieldValue(name){const field=formFields[name];if(!field)return;if(field.type==='checkbox'){field.checked=false;return}if(name!=='action_type'){field.value=''}}function setFieldVisibility(name,visible){const field=formFields[name];if(!field)return;const container=fieldContainer(field);if(container){container.hidden=!visible;container.classList.toggle('is-hidden',!visible)}}function focusFirstVisibleField(action){const order=(fieldVisibility[action]||fieldVisibility.SEND).slice();const firstName=order.find(name=>{if(name==='action_type')return false;const field=formFields[name];if(!field)return false;const container=fieldContainer(field);return !(container&&container.hidden)});const firstField=firstName?formFields[firstName]:formFields.source_network;if(firstField&&typeof firstField.focus==='function'){firstField.focus()}}function updateFieldVisibility(action){const active=new Set(fieldVisibility[action]||fieldVisibility.SEND);allIntentFields.forEach(name=>{if(name==='action_type')return;setFieldVisibility(name,active.has(name))})}function setActionTab(action){const next=fieldVisibility[action]?action:'SEND';actionButtons.forEach(button=>{button.classList.toggle('active',button.dataset.action===next)});formFields.action_type.value=next;updateFieldVisibility(next)}function updateScenarioHighlight(){scenarioButtons.forEach((button,index)=>{button.classList.toggle('scenario-selected',index===selectedScenarioIndex)})}function clearUiMessages(){document.querySelectorAll('[data-validation-message],.validation-message,.api-error,.error-message').forEach(el=>{el.textContent='';el.classList.remove('show');el.hidden=true});document.querySelectorAll('[data-risk-ack],[data-wallet-handoff],[data-completion-message],#risk-acknowledgment,#wallet-handoff,#completion-message').forEach(el=>{if('checked'in el){el.checked=false}el.textContent='';el.classList.remove('show');el.hidden=true})}function closeOpenModals(){document.querySelectorAll('dialog[open]').forEach(dialog=>{if(typeof dialog.close==='function'){dialog.close()}else{dialog.removeAttribute('open')}});document.querySelectorAll('.modal.open,.panel.open,[data-open="true"]').forEach(el=>{el.classList.remove('open');el.setAttribute('data-open','false');el.hidden=true})}function clearHiddenFieldValues(action){const active=new Set(fieldVisibility[action]||fieldVisibility.SEND);allIntentFields.forEach(name=>{if(name==='action_type')return;if(!active.has(name)){clearFieldValue(name)}})}function buildIntentFromForm(){return{action_type:nullableText(formFields.action_type.value)||'SEND',source_network:nullableText(formFields.source_network.value),destination_network:nullableText(formFields.destination_network.value),asset_symbol:nullableText(formFields.asset_symbol.value),asset_identifier:nullableText(formFields.asset_identifier.value),destination_address:nullableText(formFields.destination_address.value),expected_destination_address:nullableText(formFields.expected_destination_address.value),entered_destination_tag_or_memo:nullableText(formFields.entered_destination_tag_or_memo.value),expected_destination_tag_or_memo:nullableText(formFields.expected_destination_tag_or_memo.value),contract_address:nullableText(formFields.contract_address.value),approval_amount_or_scope:nullableText(formFields.approval_amount_or_scope.value),swap_slippage_percent:nullableNumber(formFields.swap_slippage_percent.value),transaction_origin:nullableText(formFields.transaction_origin.value),asset_was_unsolicited:Boolean(formFields.asset_was_unsolicited.checked)}}function setResultIdle(){result.className='card';result.textContent=resultDefault;cont.disabled=true;cont.textContent='Continue'}function setResultLoading(){result.className='card';result.textContent=evaluatingText;evaluateButton.disabled=true;evaluateButton.textContent='Evaluating...';cont.disabled=true;cont.textContent='Continue'}function continueLabel(decision){if(decision==='READY')return 'Continue';if(decision==='REVIEW')return 'Continue with review';return 'Continue'}function renderResult(payload){result.className='card '+payload.decision;result.innerHTML=`<h2>${payload.decision}</h2><p><b>Rule:</b> ${payload.triggered_rule_id}</p><p>${payload.explanation}</p><p><b>Recommended next step:</b> ${payload.recommended_next_step}</p>`;cont.disabled=payload.decision==='STOP';cont.textContent=continueLabel(payload.decision);evaluateButton.disabled=false;evaluateButton.textContent=evaluateDefaultLabel;result.scrollIntoView({behavior:'smooth',block:'start'})}function renderError(message){result.className='card STOP';result.innerHTML=`<h2>ERROR</h2><p>${message}</p>`;cont.disabled=true;cont.textContent='Continue';evaluateButton.disabled=false;evaluateButton.textContent=evaluateDefaultLabel;result.scrollIntoView({behavior:'smooth',block:'start'})}async function evaluateIntent(intent){setResultLoading();try{const response=await fetch('/api/evaluate',{method:'POST',cache:'no-store',headers:{'Content-Type':'application/json'},body:JSON.stringify(intent)});if(!response.ok){throw new Error(`Request failed with status ${response.status}`)}const payload=await response.json();renderResult(payload)}catch(error){renderError(`Unable to evaluate intent right now. ${error.message}`)}}async function evaluateFromForm(){await evaluateIntent(buildIntentFromForm())}function resetExperience(){form.reset();setActionTab('SEND');clearHiddenFieldValues('SEND');selectedScenarioIndex=null;updateScenarioHighlight();clearUiMessages();closeOpenModals();setResultIdle();evaluateButton.disabled=false;evaluateButton.textContent=evaluateDefaultLabel;form.scrollIntoView({behavior:'smooth',block:'start'});focusFirstVisibleField('SEND')}actionButtons.forEach(button=>{button.addEventListener('click',()=>{setActionTab(button.dataset.action||'SEND')})});checkButton.addEventListener('click',()=>form.scrollIntoView({behavior:'smooth',block:'start'}));resetButton.addEventListener('click',resetExperience);form.addEventListener('submit',async event=>{event.preventDefault();await evaluateFromForm()});setActionTab('SEND');setResultIdle();fetch('/api/scenarios',{cache:'no-store'}).then(response=>response.json()).then(scenarios=>{scenarioBox.innerHTML='';scenarioButtons=[];scenarios.forEach((scenario,index)=>{const button=document.createElement('button');button.type='button';button.textContent=`${index+1}. ${scenario.name}`;button.addEventListener('click',async()=>{selectedScenarioIndex=index;updateScenarioHighlight();fieldNames.forEach(name=>{const field=formFields[name];if(!field)return;field.value=blank(scenario.intent?.[name])});formFields.asset_was_unsolicited.checked=Boolean(scenario.intent?.asset_was_unsolicited);setActionTab(blank(scenario.intent?.action_type)||'SEND');await evaluateFromForm()});scenarioBox.appendChild(button);scenarioButtons.push(button)})}).catch(()=>{scenarioBox.textContent='Unable to load scenarios right now.'});"#;
#[cfg(test)]
mod tests {
    ...
    fn frontend_contains_required_form_and_fetch_guards() {
        ...
        assert!(
            APP_JS.contains("function resetExperience(){form.reset();"),
            "reset should call form.reset()"
        );
        assert!(
            APP_JS.contains("setActionTab('SEND');"),
