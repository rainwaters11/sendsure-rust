...
use sendsure_rust::test_support::{
    handle_accept_error, handle_client, is_ignorable_connection_error,
};
...
fn frontend_contains_required_form_and_fetch_guards() {
    ...
    assert!(
        INDEX_HTML.contains("<button type=\"button\" id=\"check\">Run preflight check</button>"),
    ...
    assert!(
        INDEX_HTML.contains("<button type=\"button\" id=\"continue\" disabled>Continue</button>"),
